...
class CNN():
  ...
  def __init__(self, process, samples = 2130):
  ...
  def get_info(self):
    ...
    df1 = pd.read_csv(self.root + '/csv/process_' + str(self.process) + '/jaffe_samples.csv')
    df2 = pd.read_csv(self.root + '/csv/process_' + str(self.process) + '/jaffe_labels.csv')
    length = len(df1)
    dataset = {}
...
class FN():
  ...
  '''
  def __init__(self, data, train_perc, verbose, process, learning_rate, type_of_loss = 0, type_of_func = 0, recover = False,  samples = 2130) :
    self.process = process
    self.data = data
    self.new_data = {}
    ...
    self.__sample = []
    self.__label = []
    self.root = 'jaffe/fully_connected_network'
  ...
  def train(self):
    ...
    while epoch < self.max_epoch and self.total_loss > min_error:
      ...
      self.train_data['inputs'] = self.train_data['inputs'][rnd]
      self.train_data['labels'] = self.train_data['labels'][rnd]
      ...
      for i in reversed(range(len(self.neurons))):
        ...
        if i == last:
          ...
          for c in range(self.neurons[last]):
            if c == label:
              self.loss[label] = (1 - result[label])**2/2
              __grad[c] = (1 - result[label])*(1 - result[label])*result[label]  # prob[label]-1 / #-prob[label]*(1-prob[label])
            else:
              __grad[c] = - result[c]*(1-result[c])*result[c] # prob[label] / #prob[label]*prob[c]
          __input = conns['o'+str(i-1)]
          # print(__grad.shape)
          ...
          der = (1-conns['o'+str(i)])*conns['o'+str(i)]
          __grad = der*(prev_grad.dot(conns['w'+str(i+1)][1:].T))
          # print(__grad.shape)
          ...
          for c in range(self.neurons[last]):
            if c == label:
              if self.type_of_loss == 0:
                ...
                __grad[c] = prob[label] - 1
              else:  
                self.loss[c] = 1 - prob[c]
              __grad[c] = -(1 - prob[label])#*prob[label]  # prob[label]-1 / #-prob[label]*(1-prob[label])
            else:
              if self.type_of_loss == 0:
                __grad[c] = prob[label]  
              else:  
                __grad[c] = prob[c]#*prob[label] # prob[label] / #prob[label]*prob[c]
          __input = conns['o'+str(i-1)]
          # print(__grad.shape)
          # if __input.max() != 0:
          #   __input = __input/__input.max()
  ...
  def save_a_sample(self):
    ...
    plt.axis('off')
    plt.colorbar()
    fig.savefig('jaffe/convolutional_network/transformed/process_' + str(self.process) + '/output.png')
...
data = test.get_info()     
net = FN(data = data, train_perc = 0.7, verbose = False, type_of_loss = type_of_loss, type_of_func = type_of_func, learning_rate = learning_rate, recover = True, process = process)
prev = net.get_epoch()
if prev:
